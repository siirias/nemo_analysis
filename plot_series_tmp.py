...
series_name='TheHindcast_{}'.format(var1)
...
var2='icevolume'  #'icecon','icethic', 'icevolume'
#gludge lines end.
...
for f, other_f in zip(files_working,other_files_working):
    ...
    if(var2 is not None):
        if(var2 in data.variables.keys()):
            ice_d=data.variables[var2][:]
            ice_d=np.ma.masked_where(ice_d<0.2,ice_d)
        else: #this one is not included, lets try to get it with gludge
            ice_d_tmp=data.variables['icethic'][:]
            ice_d=data.variables['icecon'][:]
