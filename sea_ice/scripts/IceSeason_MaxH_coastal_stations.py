...
in_dir = "/scratch/project_2002540/siiriasi/smartsea_data/derived_data/ice_data/ice_seasons/"
out_dir = "/scratch/project_2002540/siiriasi/smartsea_data/derived_data/ice_data/"

obs_dir = "/scratch/project_2002540/siiriasi/smartsea_data/derived_data/ice_data/observations/"
...
pisteet = ['Kemi','Kalajoki', 'Salgrund', 'Kylmapihlaja' ]

for piste in pisteet:
    ########## KEMI ######################
    A001=np.loadtxt('{}/Ice_season_{}_A001.txt'.format(in_dir, piste), delimiter = ",")
    B001=np.loadtxt('{}/Ice_season_{}_B001.txt'.format(in_dir, piste), delimiter = ",")
    D001=np.loadtxt('{}/Ice_season_{}_D001.txt'.format(in_dir, piste), delimiter = ",")

    A002=np.loadtxt('{}/Ice_season_{}_A002.txt'.format(in_dir, piste), delimiter = ",")
    B002=np.loadtxt('{}/Ice_season_{}_B002.txt'.format(in_dir, piste), delimiter = ",")
    D002=np.loadtxt('{}/Ice_season_{}_D002.txt'.format(in_dir, piste), delimiter = ",")

    A005=np.loadtxt('{}/Ice_season_{}_A005.txt'.format(in_dir, piste), delimiter = ",")
    B005=np.loadtxt('{}/Ice_season_{}_B005.txt'.format(in_dir, piste), delimiter = ",")
    D005=np.loadtxt('{}/Ice_season_{}_D005.txt'.format(in_dir, piste), delimiter = ",")

    HC=np.loadtxt('{}/Ice_season_{}_REANALYSIS.txt'.format(in_dir, piste), delimiter = ",")

    Obs=np.loadtxt('{}/{}_Number_of_days_with_ice.csv'.format(obs_dir, piste), delimiter = ",")
    try:
        ObsH=np.loadtxt('{}/{}_maxH.csv'.format(obs_dir, piste), delimiter = ",")
    except OSError: #no measuring data exists
        print("No measured data for ice thickness for:{}".format(piste))
        ObsH = None
    ...
    Annual_max_HC=[]
    Ice_days_HC=[]
    for i in range(ref_start, scen_end+1):
    ...
    for i in range(hind_start,hind_end+1):
    ...
    x1=np.array(range(ref_start,scen_end+1))
    x2=np.array(range(hind_start, hind_end+1))

    data_=[x1,Ice_days_A002,Ice_days_B002,Ice_days_D002]
    data_=np.array(data_)
    np.savetxt('{}/Ice_season_L_{}_ABD002.txt'.format(out_dir, piste),data_.T)

    data_=[x1,Ice_days_A005,Ice_days_B005,Ice_days_D005]
    data_=np.array(data_)
    np.savetxt('{}/Ice_season_L_{}_ABD005.txt'.format(out_dir, piste),data_.T)

    data_=[x1,Annual_max_A002,Annual_max_B002,Annual_max_D002]
    data_=np.array(data_)
    np.savetxt('{}/AnnualMax_H_{}_ABD002.txt'.format(out_dir, piste),data_.T)

    data_=[x1,Annual_max_A005,Annual_max_B005,Annual_max_D005]
    data_=np.array(data_)
    np.savetxt('{}/AnnualMax_H_{}_ABD005.txt'.format(out_dir, piste),data_.T)


    data_=[x2,Ice_days_HC]
    data_=np.array(data_)
    np.savetxt('{}/Ice_season_L_{}_HC.txt'.format(out_dir, piste),data_.T)

    data_=[x2,Annual_max_HC]
    data_=np.array(data_)
    np.savetxt('{}/AnnualMax_H_{}_HC.txt'.format(out_dir, piste),data_.T)


    max_scen_len = min((len(A002),len(B002),len(D002)))
    H_002=[(A002[i,3]+B002[i,3]+D002[i,3])/3 for i in range(max_scen_len)]
    max_scen_len = min((len(Annual_max_A002),len(Annual_max_B002),len(Annual_max_D002)))
    Max_H_002=[(Annual_max_A002[i]+Annual_max_B002[i]+Annual_max_D002[i])/3 for i in range(max_scen_len)]
    ice_days_002=[(Ice_days_A002[i]+Ice_days_B002[i]+Ice_days_D002[i])/3 for i in range(max_scen_len)]

    max_scen_len = min((len(A005),len(B005),len(D005)))
    H_005=[(A005[i,3]+B005[i,3]+D005[i,3])/3 for i in range(max_scen_len)]
    max_scen_len = min((len(Annual_max_A005),len(Annual_max_B005),len(Annual_max_D005)))
    Max_H_005=[(Annual_max_A005[i]+Annual_max_B005[i]+Annual_max_D005[i])/3 for i in range(max_scen_len)]
    ice_days_005=[(Ice_days_A005[i]+Ice_days_B005[i]+Ice_days_D005[i])/3 for i in range(max_scen_len)]
    ...
    if(ObsH is not None):
        z_mean_obsH=np.polyfit(ObsH[np.isfinite(ObsH[:,1]),0], ObsH[np.isfinite(ObsH[:,1]),1]/100,1)
